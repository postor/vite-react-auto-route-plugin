...
async function getCode(folder: string, skip: RegExp, extension: string, resourcePath: string) {
  files.add(resourcePath)
  let { pathNameDic, routes } = await getMeta(folder, skip, extension), lines = [
  ...
  for (let [file, name] of pathNameDic.entries()) {
    ...
    json = json.replaceAll(JSON.stringify(file), `React.createElement(${name})`)
  }
  json = json.replaceAll(`"element": ""`, `"element":React.createElement(Outlet)`)
  lines.push(``)
...
async function getMeta(folder: string, skip: RegExp, extension: string) {
  let usedNames = new Set, pathNameDic: Map<string, string> = new Map
